...
class FramesIndex extends Component {
  constructor(props) {
    super(props);
  }
  ...
  renderFrames() {
    ...
    return _.map(this.props.frames, frame => {
      ...
      const panel_rate = accounting.formatMoney(frame.panel_rate, "$ ", 0);
      const lat = frame.lat;
...
function mapStateToProps({ frames }) {
  return { frames };
}

export default connect(mapStateToProps, { fetchFrames })(FramesIndex);
